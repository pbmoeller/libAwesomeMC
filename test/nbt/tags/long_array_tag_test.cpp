...
TEST(LongArrayTag, Constructor)
{
    ...
    EXPECT_EQ(amc::TagType::LongArray, longArrayTag.getType());
...
{
    amc::LongArrayTag longArrayTag("TagName");
...
TEST(LongArrayTag, Constructor_3)
{
    ...
    amc::LongArrayTag longArrayTag(value);
    EXPECT_STREQ("", longArrayTag.getName().c_str());
    EXPECT_EQ(amc::TagType::LongArray, longArrayTag.getType());
    std::vector<int64_t> data = longArrayTag.getValue();
...
{
    std::vector<int64_t> value = {-123, 0, 127};
    amc::LongArrayTag longArrayTag("TagName", value);
    EXPECT_STREQ("TagName", longArrayTag.getName().c_str());
    EXPECT_EQ(amc::TagType::LongArray, longArrayTag.getType());
    std::vector<int64_t> data = longArrayTag.getValue();
    EXPECT_EQ(3, data.size());
...
TEST(LongArrayTag, MoveConstructor)
{
    ...
    // Test Move
    amc::LongArrayTag tagB(std::move(tagA));
...
{
    amc::LongArrayTag longArrayTagA("Test1", {1, 2, 3});

    amc::LongArrayTag *longArrayTagB = tag_cast<amc::LongArrayTag*>(longArrayTagA.clone());
...
{
    amc::LongArrayTag longArrayTagA("Test1", {1, 2, 3});
    amc::LongArrayTag longArrayTagB("Test1", {1, 2, 3});
    amc::LongArrayTag longArrayTagC("Test1", {1, 2, 3});
    amc::LongArrayTag longArrayTagD("Test2", {1, 2, 5});
    amc::LongArrayTag longArrayTagE;
    amc::EndTag endTag;
...
TEST(LongArrayTag, getData)
{
    ...
    amc::LongArrayTag longArrayTag("longArrayTag", std::vector<int64_t>({-1000001, 4, 123456789}));
...
{
    amc::LongArrayTag longArrayTagA("TestTag", {1, 2, 3});
    // Test index too high
    EXPECT_FALSE(longArrayTagA.eraseAt(3));
    EXPECT_FALSE(longArrayTagA.eraseAt(900));

    // successively erase first index until empty
    ASSERT_FALSE(longArrayTagA.isEmpty());
    EXPECT_TRUE(longArrayTagA.eraseAt(0));
    EXPECT_TRUE(longArrayTagA.eraseAt(0));
    EXPECT_TRUE(longArrayTagA.eraseAt(0));
    ASSERT_TRUE(longArrayTagA.isEmpty());

    // Expect false, when trying to erase from empty ByteAraryTag
    EXPECT_FALSE(longArrayTagA.eraseAt(0));
...
TEST(LongArrayTag, insert)
{
    ...
    amc::LongArrayTag longArrayTagB("TestTag", {1, 3});
    ASSERT_EQ(2, longArrayTagB.size());
...
TEST(LongArrayTag, at)
{
    ...
    // Test copy
    amc::LongArrayTag longArrayTagA("TestTag", {value1_in, value2_in, value3_in});
    ...
    // Test reference
    amc::LongArrayTag longArrayTagB("TestTag", {value1_in});
...
TEST(LongArrayTag, at_const)
{
    ...
    // Test const copy
    const amc::LongArrayTag longArrayTagA("TestTag", {value1_in, value2_in, value3_in});
    ASSERT_EQ(3, longArrayTagA.size());
...
{
    amc::AbstractTag *testTag = new amc::LongArrayTag("A", std::vector<int64_t>({1, 2, 3}));

    amc::LongArrayTag *otherTag = tag_cast<amc::LongArrayTag*>(testTag);

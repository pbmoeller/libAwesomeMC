...
{
    assert(n <= 16);
    return n == 16 ? std::numeric_limits<uint16_t>::max() : ((int16_t)1 << n) - 1;
}

inline uint32_t setNLeastSignificantBits32(char n)
{
    assert(n <= 32);
    return n == 32 ? std::numeric_limits<uint32_t>::max() : ((int32_t)1 << n) - 1;
}

inline uint64_t setNLeastSignificantBits64(char n)
{
    assert(n <= 64);
    return n == 64 ? std::numeric_limits<uint64_t>::max() : ((int64_t)1 << n) - 1;
}
